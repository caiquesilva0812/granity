    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4" style="background: rgba(0,0,0,0.55);">
      ...
      >
        <div class="flex items-center justify-between px-5 lg:px-6 py-4 border-b shrink-0" :style="{ borderColor: 'var(--border)' }">
          <div>
            <h2 class="font-semibold text-base" :style="{ color: 'var(--text)' }">Editar Bloco</h2>
            <p class="text-xs mt-0.5 font-mono" :style="{ color: 'var(--text-muted)' }">{{ block.code }}</p>
          </div>
        ...
        <div class="overflow-y-auto flex-1 p-5 lg:p-6 space-y-5">
          ...
          <div>
            <label class="block text-sm font-medium mb-1.5" :style="{ color: 'var(--text)' }">Dimensões do Bloco (metros) <span class="text-red-400">*</span></label>
            ...
            <div v-if="calculatedVolume > 0" class="mt-3 flex items-center gap-2 px-4 py-2.5 rounded-lg border"
          ...
          <div>
            <label class="block text-sm font-medium mb-3" :style="{ color: 'var(--text)' }">Fotos do Bloco</label>
        ...
        <div class="flex items-center gap-3 px-5 lg:px-6 py-4 border-t shrink-0" :style="{ borderColor: 'var(--border)' }">
          <button type="button" @click="$emit('save', form)"
            class="px-5 py-2.5 bg-primary hover:bg-primary-hover text-white text-sm font-medium rounded-lg transition-colors">
            Salvar Alterações
          </button>
...
import { X, Box, Camera, ImagePlus } from "lucide-vue-next";
...
defineEmits<{ close: []; save: [form: object] }>();

function parseDimensions(dim: string) {
  const match = dim.replace(/,/g, ".").match(/([\d.]+)\s*[×x]\s*([\d.]+)\s*[×x]\s*([\d.]+)/);
  if (!match) return { length: null as number | null, width: null as number | null, height: null as number | null };
  return { length: parseFloat(match[1]), width: parseFloat(match[2]), height: parseFloat(match[3]) };
}

const parsed = parseDimensions(props.block.dimensions);
const form = reactive({
  date: props.block.date.split("/").reverse().join("-"),
  front: props.block.front,
  length: parsed.length,
  width: parsed.width,
  height: parsed.height,
  classification: props.block.classification,
